!function() {
	...
	var center_redraw = function() {
	    ...
	    dfx_text
		.attr("x", scale_point(ex.eval_fn(tangent_fn, center.x + 0.5)).x)
		.attr("y", scale_point(ex.eval_fn(tangent_fn, center.x + 0.5)).y -15)
		.text("slope: f'(x0)");

	    secant_text
		.attr("x", scale_point(ex.eval_fn(secant_fn, center.x - 0.50)).x)
		.attr("y", scale_point(ex.eval_fn(secant_fn, center.x - 0.5)).y -15)
		.attr("align", "right")
		.text("slope: [f(x0+dx)-f(x0)] / dx");
	} /*center_redraw*/
	...
	/* text element - derivative */
	var dfx_text = null;
	/* text element - secant (fwd difference) */
	var secant_text = null;

	/* establish initial values for lo_x, lo_pt, hi_x, hi_pt, tangent_fn, secant_fn 
	 * doesn't do any redrawing
	 */
	...
	dfx_text = box.selectAll("." + "dfx_text")
	    .data([{pt: scale_point(ex.eval_fn(tangent_fn, 0.25)),
		    text: "slope: f'(x0)"}])
	    .enter()
	    .append("svg:text")
	    .attr("class", "dfx_text")
	...
	secant_text = box.selectAll("." + "secant_text")
	    .data([{pt: scale_point(ex.eval_fn(secant_fn, -0.25)),
		    text: "slope: [f(x0+dx)-f(x0)]/dx"}])
	    .enter()
	    .append("svg:text")
	    .attr("class", "secant_text")
	    .attr("x", function(d) { return d.pt.x; })
	    .attr("y", function(d) { return d.pt.y; })
	    .text(function(d) { return d.text; })
	    .attr("font-family", "sans-serif")
